...
iftImage *iftCompute_ISF_GRID_MEAN_Superpixels(iftImage *img, int nsuperpixels, float alpha, float beta, int niters, int smooth_niters, int *nseeds, int *finalniters) {
...
int main(int argc, char *argv[]) 
{
  ...
  if (argc!=8)
    iftError("Usage: iftISF_GRID_MEAN <image.[pgm,ppm,png]> <nsamples> <alpha (e.g., [0.005-0.2])> <beta (e.g., 12)> <niters (e.g., 10)> <smooth niters (e.g., 2)> <output_image>","main");
  
  img  = iftReadImageByExt(argv[1]);
  
  normvalue =  iftNormalizationValue(iftMaximumValue(img)); 
  
  label     = iftCompute_ISF_GRID_MEAN_Superpixels(img, atoi(argv[2]), atof(argv[3]), atof(argv[4]), atoi(argv[5]), atoi(argv[6]), &(nseeds), &(niters));
